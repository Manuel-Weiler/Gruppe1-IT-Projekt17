...
public class PartnerprofilWidget extends Composite {
	...
	public PartnerprofilWidget() {
		...
		// Als Nutzer der Plattform muss mindestens ein Personen-Konto angelegt
		// werden.
		typbox.addItem("Person");
		typbox.setVisibleItemCount(1);

		// Dem Nutzer werden beispielhafte Eigenschaften zum Ausfüllen
		// vorgeschlagen.
	...
	public PartnerprofilWidget(Organisationseinheit o) {
		...
		// Clickhandler zum Löschen des eigenen Benutzerprofils des eingeloggten
		// Nutzers
		...
		Project4uVerwaltung.getEigenschaftenOfOrganisationseinheit(o, new AsyncCallback<Vector<Eigenschaft>>() {
			...
			public void onSuccess(Vector<Eigenschaft> result) {
				...
				// Über eine DialogBox kann der Nutzer weitere Eigenschaften
				// hinzufügen
						...
						ok.addClickHandler(new ClickHandler() {
							...
							public void onClick(ClickEvent event) {
								if (name.getValue().isEmpty() || wert.getValue().isEmpty()) {
									...
									// Die neue Eigenschaft wird direkt in der
									// Datenbank gespeichert
									Project4uVerwaltung.insertEigenschaft(eig, neuesProfil,
											new AsyncCallback<Eigenschaft>() {
										...
										public void onSuccess(Eigenschaft result) {
											...
											// Danach wird das
											// PartnerprofilWidget neu geladen
											Project4uVerwaltung.getOrganisationseinheitByUser(
				...
				HTML hr = new HTML(
						"<hr style= 'border: 0; height: 3px; background: #333; margin-top: 50px; margin-bottom: 15px; background-image: linear-gradient(to right, #ccc, #333, #ccc);'>");
				te.add(hr);
				 ...
				 * durchzuführen.
				 */
				Project4uVerwaltung.getAllOrganisationseinheitenOfTypTeamUnternehmen(
	...
	private CellTable<Eigenschaft> createCellTable(Vector<Eigenschaft> eigenschaften) {
		...
		// Die buttomColumn soll ein einfaches Löschen der jeweiligen
		// Eigenschaft direkt über den Button erlauben.
			...
			public void update(int index, Eigenschaft object, String value) {
				...
				Project4uVerwaltung.deleteEigenschaft(object, new AsyncCallback<Void>() {
					...
					public void onSuccess(Void result) {
						...
						// Nach dem Löschen einer Eigenschafte wird das
						// PartnerprofilWidget neu erzugt und angezeigt.
		...
		// Die Daten der Eigenschaftsobjekte können direkt über die
		// Tabellenzellen bearbeitet werden.
